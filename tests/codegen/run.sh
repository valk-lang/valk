...
echo "> Export externally visible symbols under their declared names"

export_ir="$workdir/export.ll"
out=$("$VALK" build "$DIR/export.valk" --ir --no-warn -o "$export_ir" 2>&1)
status=$?
if [ "$status" -ne 0 ]; then
    echo "# Failed to build export IR fixture"
    echo "$out"
    exit 1
fi
...
if ! grep -q '^define dso_local i64 @"exported_function"' "$export_ir" \
    ...
    echo "# Exported definitions did not use external linkage and their declared names"
    cat "$export_ir"
    exit 1
fi

if ! grep -q '^define dso_local .*__export_helper__' "$export_ir" \
    || grep -q 'unreachable_function' "$export_ir" \
    || grep -q 'unreachable_global' "$export_ir"; then
    echo "# Export reachability did not retain only the exported symbol's dependency graph"
    cat "$export_ir"
    exit 1
fi
