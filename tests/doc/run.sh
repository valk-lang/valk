...
if [[ "$stdlib_markdown" != *'+ extend &[T] {'* ]] \
    ...
    || [[ "$stdlib_markdown" != *'+ fn read(fd: i32, buf: mut &[u8], offset: uint (uint.$max)) uint !IoError'* ]] \
    || [[ "$stdlib_markdown" != *'+ fn read_sync(fd: i32, buf: mut &[u8], offset: uint (uint.$max)) uint !IoError'* ]] \
    || [[ "$stdlib_markdown" != *'+ fn read(buf: mut &[u8]) uint !io:IoError'* ]] \
    || [[ "$stdlib_markdown" != *'+ fn write(fd: i32, data: &[u8], offset: uint (uint.$max)) uint !IoError'* ]] \
