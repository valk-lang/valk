...
cases="$cases fixed-range-copy-bounds:1 fixed-range-view-bounds:1"
cases="$cases endian-short-read:1:Needs endian-short-write:1:Needs"
...
cases="$cases round-up-overflow:1:outside round-down-overflow:1:outside"
...
run_case() {
    ...
    if [ "$name" = "panic-in-library" ] && [[ "$output" != *"Index out of bounds at src/core/ByteBuffer.valk:124 in package valk"* ]]; then
