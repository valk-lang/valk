...
method on a read-only borrow is read-only too. `local T` stays the exclusive
...
`local` borrow of it, passing it to a `&mut` or `local` parameter, and calling
...
Struct method receivers are stack borrows: `this` is `local T`. Calling a
...
- `&mut T` converts to `local T` in place; the layouts are the same and only
...
function that writes but does not keep the borrow takes `local T`, and one
