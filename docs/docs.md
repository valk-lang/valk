...
Integer literals: `255`, hex `0xff`, octal `0c377`, binary `0b1111_1111`. A leading
zero is still decimal (`017` is 17).
...
to the end. An entry is one element, as in `Array[T]`, so a tuple element is
written as it is and never spread: `[(u8, u64) x 1] = { pair() }`, and
`[u8 x 2] = { two_bytes() }` is rejected because a `(u8, u8)` is not a `u8`.
...
{
    "vars": {
        "TEST_FLAGS": "--test --def \"GC_DEBUG=1\"",
        "TARGETS": ["linux-x64", "macos-x64", "win-x64"]
    },
    "make": {
        "hello": { "dir": "src", "args": "-o ./hello", "global": true },
        "test": {
            "needs": ["hello"],
            "cmd": [
                "./valk build ./tests $TEST_FLAGS -o ./debug/test",
                "./debug/test"
            ]
        },
        "lint-all": "for t in $TARGETS; do valk build ./src --lint --target $t || exit $?; done"
    }
}
```
...
```json
{
    "make": {
        "clean": [
            { "win": "rmdir /s /q debug", "default": "rm -rf ./debug" },
            "echo cleaned"
        ]
    }
}
...
The keys are `linux`, `macos` and `win`, an operating system with its architecture
(`macos-arm64`, `win-x64`, …) and `default`; the most exact one wins. A line that says
nothing about the system it runs on is left out, so a step meant for one platform needs no
`default`.
...
`T` must be a class type. Waiting for the lock yields to other coroutines on the thread, like `core.Mutex`. Taking a free lock is a single atomic operation, and locks, channels and cancel tokens hold no file descriptors, so a program can create as many as it likes. The lock is not reentrant: locking the same `Lock` again inside its own `lock` block deadlocks. Another coroutine on the same thread that locks it just waits until the block ends, as a coroutine on another thread would. Reading the value outside a `lock` block is not possible; every reader takes the lock too. A thread whose entry function returns while one of its coroutines is still inside a `lock` block keeps running its coroutines until that block ends.
