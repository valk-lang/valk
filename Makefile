...
test: valk
	mkdir -p ./debug
	./valk build ./tests/*.valk . --test $(FLAGS) -o ./debug/test-all --def "GC_DEBUG=1" -vv
	./debug/test-all
	@./tests/compile-errors/run.sh

test-win: valk
	mkdir -p ./debug
	./valk build ./tests/*.valk . --test -vv -o ./debug/test-win.exe --target win-x64 $(FLAGS) --def "GC_DEBUG=1"
	./debug/test-win.exe
