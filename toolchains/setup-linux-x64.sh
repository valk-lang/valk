...
LLVM_LIN_X64="$LIB_DIR/linux-llvm-22-x64"
...
if [ ! -d "$LLVM_LIN_X64" ]; then
	...
	cd $LIB_DIR
	wget "https://files.valk-cdn.dev/toolchains/linux-llvm-22-x64.tar.gz"
	tar -xf "linux-llvm-22-x64.tar.gz" --checkpoint=.100
	rm "linux-llvm-22-x64.tar.gz"
fi
